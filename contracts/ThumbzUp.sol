  ...
  ReentrancyGuard
{
  ...
  function setProvenanceHash(string memory provenanceHash_) public onlyOwner {
    require(bytes(_provenanceHash).length == 0, "Provenance hash already set");
    _provenanceHash = provenanceHash_;
    emit ProvenanceHashSet(provenanceHash_);
  }
  ...
  function collectReserves() public onlyOwner {
    require(bytes(_provenanceHash).length > 0, "Provenance not set");
  ...
  function publicMint(uint256 count) public payable nonReentrant {
    require(_whitelistMerkleRoot == 0, "Whitelist sale is still active");
  ...
  function isApprovedForAll(
    address _owner,
    address operator
  ) public view override(IERC721, ERC721, ERC721OpenSeaGassLess) returns (bool) {
